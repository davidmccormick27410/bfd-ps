...
$datasource = "(DESCRIPTION =
               ...
               (ADDRESS = 
               (PROTOCOL = TCP)
               (HOST = hostingdb0p.its.unc.edu)(PORT = 1521)))
               (CONNECT_DATA =
               (SERVICE_NAME = ldstrprd.unc.edu)))"

$username = "pwrline"
$password = Get-Content C:\UNC\lodestar.txt

$queryStatment = "SELECT uidmeter, starttime, stoptime, servicetypecode FROM meterhistory" #Be careful not to terminate it with a semicolon, it doesn't like it

$destinationInstance = "localhost"
$destinationDB = "BFD"
$destinationSchema ="Staging"
$destinationTable = "LS_METER_HISTORY"
